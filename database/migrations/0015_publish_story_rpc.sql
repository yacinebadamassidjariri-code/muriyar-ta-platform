...
begin
  ...
  if v_caller_supplied_slug then
    v_slug := v_base_slug;
    if exists (select 1 from public.published_stories where public.published_stories.slug = v_slug) then
      raise exception 'slug_taken' using errcode = '23505';
    end if;
  else
    v_slug := v_base_slug;
    while exists (select 1 from public.published_stories where public.published_stories.slug = v_slug) loop
      v_n := v_n + 1;
